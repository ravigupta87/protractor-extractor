...
Tree.prototype.add = function(data, toData, traversal) {
    var child = new Node.Node(data),
        parent = null,
        callback = function(node) {
            if (node.data.type === toData.type && node.data.name === toData.name) {
                parent = node;
            }
        };
...
Tree.prototype.remove = function(data, fromData, traversal) {
    ...
    var callback = function(node) {
        if (node.data.type === fromData.type && node.data.name === fromData.name) {
            parent = node;
        }
    };
